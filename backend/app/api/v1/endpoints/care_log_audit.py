...
권한: 조회·업로드·삭제 모두 관리자급(role ADMIN 또는 position 시설장)만. 직원 개인별 오류 집계라 그 외에는 보이지 않는다.
"""
...
def _manager(current_user: User = Depends(get_current_user)) -> User:
    ...
    App.tsx 의 ManagerRoute 와 같은 기준이다."""
    role = current_user.role.value if hasattr(current_user.role, "value") else str(current_user.role)
    pos = getattr(current_user, "position", None) or ""
    if role != "ADMIN" and pos != "시설장":
        raise HTTPException(403, "주간 기록지 점검은 관리자·시설장만 볼 수 있습니다.")
    return current_user
...
@router.get("/weeks")
def list_weeks(db: Session = Depends(get_db), _: User = Depends(_manager)):
    """점검 주 목록 — 최신 우선."""
...
def get_week(week_start: str, db: Session = Depends(get_db), _: User = Depends(_manager)):
