...
# 치료사(물리·작업)도 같다 — 치료 일정을 잡으려면 그날 어느 손이 나오는지
# 봐야 한다. 보기만 열어 둔다. 편성·메모·잠금·버전은 그대로 시설장 손에 있다.
EDIT_POSITIONS = ("시설장",)
VIEW_POSITIONS = ("시설장", "대표", "이사", "사회복지사", "물리치료사", "작업치료사")
...
def export_schedule(month: str = Query(...), db: Session = Depends(get_db), _: User = Depends(_viewer)):
    ...
    # ── 범례 (원본 하단) ──
    LEGEND = [
        ("D", "08:50 ~ 18:00"), ("M", "06:50 ~ 16:00"), ("N", "18:00 ~ 익일 09:00"),
