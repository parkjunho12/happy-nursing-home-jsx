...
# 직원 점심은 배식 시각이 아니라 '아침에서 오후로 넘어가는 시각' 으로 가른다.
# 식사 시간 설정을 쓰면 어르신 식수 정산 값이 바뀔 때 직원 인원이 조용히 달라진다.
LUNCH, BREAKFAST = smeal.NOON_SPLIT, 8 * 60
eq(smeal.NOON_SPLIT, 12 * 60 + 30, "오전↔오후 경계는 12:30")
eq(smeal.NOON_SPLIT_LABEL, "12:30", "경계 표기")
...
# 12:30 에 끝나는 근무는 마치고 나가시는 중이라 세지 않는다.
# 반대로 12:30 을 품으면 짧아도 센다 — 몇 시간인지가 아니라 그 시각에 계시는지다.
for code, want, why in [
    ("0930~1230", False, "12:30 에 끝난다 — 나가시는 중"),
    ("0930 1230", False, "같은 근무, 공백 표기"),
    ("1000-1300", True, "3시간이지만 12:30 을 품는다"),
    ("1130~1400", True, "2시간 30분이지만 12:30 을 품는다"),
    # 선에서 끝나면 나가시는 중이고, 선에서 시작하면 오신 것이다 —
    # 배식하는 순간 한 분은 없고 한 분은 계신다
    ("1230~1700", True, "12:30 에 시작 — 그 자리에 오셨다"),
    ("D", True, "주간"), ("M", True, "모닝"), ("AD", True, "오전"),
    ("PD", False, "오후 13:30 출근"), ("N", False, "야간"),
]:
    eq(smeal.at_meal(code, LUNCH), want, f"12:30 점심 — {why}")

# 빠진 까닭을 사람 말로 적는다 — 숫자만 주면 근무표를 다시 펴 보게 된다
eq(smeal.why("休", LUNCH), "연차", "사유 — 연차")
eq(smeal.why("N", LUNCH), "그 시각엔 아직 출근 전(N)", "사유 — 야간")
eq(smeal.why("0930~1230", LUNCH), "그 시각엔 이미 퇴근(0930~1230)", "사유 — 점심 전에 나가심")
eq(smeal.why("D", LUNCH), "", "세는 근무는 까닭이 없다")
eq(smeal.why("N", LUNCH), "그 시각엔 아직 출근 전(N)", "사유 — 야간")
