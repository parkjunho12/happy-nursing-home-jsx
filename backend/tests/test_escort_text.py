...
# ── ②-3 주민등록번호는 업체 글에만, 적힌 경우에만 ─────────────────────────
# 업체가 병원 접수를 대신할 때만 요구하는 값이다. 늘 필요한 게 아니므로
# 빈 칸을 '확인 필요' 로 되묻지 않고, 복지팀이 적은 경우에만 내보낸다.
WITH_RRN = {**FULL, "resident_rrn": "500101-1234567"}
has(vendor_text(WITH_RRN), "· 주민등록번호: 500101-1234567", "업체 글의 주민번호")
eq("주민등록번호" in v, False, "안 적으면 그 줄이 아예 없다")
eq("500101" in request_text(WITH_RRN), False, "톡방 글에는 주민번호가 없다")
eq("주민등록번호" in decision_text({**WITH_RRN, "transport": "보호자 차량"}), False,
   "확정 글(톡방)에도 주민번호가 없다")
...
print("✅ 병원동행 글 정상 — 여섯 항목 차례 12건 · 빈 칸 표기 7건 "
      "· 요일 2건 · 업체 글 13건 · 주민번호 4건 · 확정 글 7건 · 필수 칸 5건 · 권한 51건")
