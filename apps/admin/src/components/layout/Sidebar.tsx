...
const ADMIN_ROLES = ['ADMIN', 'MANAGER', '대표', '시설장', '사무국장']
...
const Sidebar = () => {
  ...
  const isAdmin = ADMIN_ROLES.includes(role) || ADMIN_ROLES.includes(position)

  const activeResidents = residents.filter(r => r.status === 'active').length
  const activeStaff = staffList.filter(s => s.status === 'active').length
  const todayTodo = checklists.filter(
  ...
  const adminNav = [
    ...
    {
      to: '/analytics/page-views',
      icon: LayoutDashboard,
      label: '페이지뷰 통계',
    },
    {
      to: '/analytics/suspicious-ips',
      icon: ShieldCheck,
      label: '의심 IP 통계',
    },
    {
  ...
  const sectionButtonClass =
    'flex items-center justify-between w-full px-4 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wider hover:text-gray-600'
