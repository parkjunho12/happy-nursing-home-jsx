...
> 🤖 **이 PC에서 Claude Code로 설치하실 거라면** 같은 폴더의 `CLAUDE.md` 를 보세요.
> 단계별 실행·성공 판정·실패 분기가 작업 지시서 형태로 정리돼 있습니다.
> (Claude Code는 작업 폴더의 `CLAUDE.md` 를 자동으로 읽습니다)
...
## 8-1. 시계 맞추기 (방송 시각이 어긋나면)

방송은 '몇 시 몇 분'에 나가야 하는 일이라 시계가 틀어지면 그만큼 어긋납니다.

- **예약 방송**은 이 PC 의 시계를 기준으로 나갑니다. 서버 시계가 틀어져 있어도
  현장에서는 제 시각에 나갑니다.
- **즉시 방송**은 시계와 무관하게 받는 즉시 나갑니다.
- 서버와 차이가 30초 이상이면 Admin 현황 화면에 `⏱ 시계 N분 차이` 배지가 뜨고,
  Agent 로그에도 경고가 남습니다.
...
배지가 계속 보이면 **서버 쪽 시계**가 틀어진 것일 수 있습니다. 그 경우
기록 시각(생성일시·방송 이력)이 전부 어긋나므로 서버에서도 동기화를 확인하세요.

---

## 9. 방송 PC의 IP는 몰라도 됩니다

서버가 방송 PC로 접속하지 않기 때문입니다.

```
서버 ──X──> 방송 PC    (서버가 먼저 접속하는 일이 없습니다)
서버 <───── 방송 PC    (PC가 30초마다 서버로 물어봅니다)
```

방송 PC가 서버에 **먼저 접속해서** 가져갑니다 — heartbeat(30초), 동기화(5분),
그리고 시각이 되면 claim. 즉시 중지 같은 명령도 서버가 큐에 넣어두면
PC가 heartbeat 때 받아갑니다.

그래서 **고정 IP·포트포워딩·DDNS·방화벽 인바운드 규칙이 전혀 필요 없습니다.**
요양원 공유기 뒤 사설 IP(192.168.x.x)여도, 공유기에서 IP가 바뀌어도 그대로 동작합니다.
기기 식별은 IP가 아니라 등록할 때 정한 `device_id` + 기기 토큰으로 합니다.

**그래도 그 PC를 찾아가야 할 때**(원격 데스크톱, 현장 점검)를 위해
Agent가 스스로 알아낸 호스트명과 원내 IP를 heartbeat마다 보고합니다.
Admin → 방송 관리 → 현황 → 방송 PC 목록에서 볼 수 있습니다.

| 표시 | 뜻 |
|---|---|
| `192.168.0.42` (회색 배지) | 원내 網 주소 — 같은 네트워크에서 원격 접속할 때 씁니다 |
| `NURSING-PC01` | PC 호스트명 |
| 접속 IP (배지 툴팁) | 서버가 본 주소 = 요양원 공유기의 WAN IP. 어느 지점인지 확인용 |
...
| 증상 | 확인할 것 |
|---|---|
| Admin에 PC가 안 보임 | `register` 를 했는지, `server_url` 이 맞는지, PC에서 서버로 **나가는** 방향(443) 이 막혔는지 |
...
| 방송 시각이 어긋남 | **서버나 이 PC 의 시계가 틀어진 것**입니다. Admin → 방송 관리 → 현황의 PC 줄에 `⏱ 시계 N분 차이` 배지가 뜹니다. 예약 방송은 이 PC 시계를 기준으로 나가므로 **이 PC 시간부터** 맞추세요 — Windows: `w32tm /resync`, Linux: `sudo timedatectl set-ntp true`. 서버 쪽도 함께 확인해야 기록 시각이 맞습니다 |
