...
| 방송이 몇 분 늦게 나감 | **PC 시계가 서버와 어긋난 것**입니다. 로그에 `PC 시계가 서버와 N초 느립니다` 경고가 있는지 보세요. Agent 가 보정해 내보내지만 근본 해결은 시간 동기화입니다 — Windows: 설정→시간 및 언어→"지금 동기화", Linux: `sudo timedatectl set-ntp true` |
